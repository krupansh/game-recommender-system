...
# Class to define the data and validation provided as input to the API for GET, POST
...
# Class to define the data and validation when using PATCH API
class GameUpdate(BaseModel):
    name: Union[str, None] = None # None allows to provide empty values in the data/variable
...
# helper function to save the new data to CSV and Pickle
def save_data(df):
...
# helper function that retrains the recommender system with new data
def retrain_recommender():
    from recommender import GameRecommender
    GameRecommender(pickle_path=PKL_PATH)

# helper function that validates the data before saving
...
def get_all_games():
    ...
    # Output provided in dictionary, FASTAPI will automatically parse it into JSON format
    return df.to_dict(orient="records")

# Route to add new game data to the database
...
def create_game(game: Game):
    ...
    save_data(df)
    # retrain the recommender after saving the data
    retrain_recommender()

    # Output provided in dictionary, FASTAPI will automatically parse it into JSON format
    return {"message": "Game added successfully.", "id": new_id}

# Route to update existing game data in the database
...
def update_game(game_id: int, game_update: GameUpdate):
    ...
    # validation to see if the data exists
    ...
    # validating the new data for update
...
def delete_game(game_id: int):
    df = load_data()
    # validation to see if data exists
    if game_id not in df["id"].values:
        raise HTTPException(status_code=404, detail="Game not found.")
    df = df[df["id"] != game_id]
    save_data(df)
    # return the recommender after deleting the data
    retrain_recommender()
    # Output provided in dictionary, FASTAPI will automatically parse it into JSON format
